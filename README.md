...
This repo is a custom Harbor evaluation exploring how different models handle strict API schemas. Specifically, it tests for a failure mode I call **Type-Blind Tool Looping**.

## The Scenario
When a tool throws a formatting error (like requiring `YYYY-MM-DD` instead of "tomorrow"), what does the model do? 

A resilient agent reads the error, adjusts its payload, and tries again. A susceptible agent ignores the feedback and just hammers the API with the exact same bad payload until it times out.
